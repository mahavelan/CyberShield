# app.py
"""
CYBERSHIELD — Final Web App
- Supports Numerical, Text, and Image datasets
- Auto-detect dataset type
- Supervised vs Unsupervised handled
- Hybrid models (multiple) allowed only if checkbox ON
- Multiple visualizations allowed only if checkbox ON
- Metrics, confusion matrix, graphs, and CSV download
"""
...
try:
    ...
    from tensorflow.keras.layers import Dense, Input, Dropout, Conv1D, Conv2D, MaxPooling1D, MaxPooling2D, Flatten, LSTM, Embedding, Bidirectional
...
if dtype == "Numerical":
    models = ["Logistic Regression","Random Forest","Decision Tree","KNN","SVM","Naive Bayes","Gradient Boosting","AdaBoost","MLP","Keras-MLP","Keras-1D-CNN"]
elif dtype == "Text":
    models = ["Naive Bayes (Text)","Logistic Regression (Text)","SVM (Text)","Random Forest (Text)","LSTM","BiLSTM","Text-CNN","BERT-lite","AdaBoost (Text)","Gradient Boosting (Text)"]
elif dtype == "Image":
    models = ["CNN-2D","ResNet50","MobileNetV2","VGG16","LeNet","Custom CNN","Autoencoder","DenseNet","InceptionV3","Xception"]
        ...
        for m in chosen:
            try:
                if m == "Logistic Regression":
                    ...
                    clf = GradientBoostingClassifier().fit(X_train,y_train); pred = clf.predict(X_test)
                elif m == "AdaBoost":
                    clf = AdaBoostClassifier().fit(X_train,y_train); pred = clf.predict(X_test)
                elif m == "MLP":
                    from sklearn.neural_network import MLPClassifier
                    clf = MLPClassifier(max_iter=500).fit(X_train,y_train); pred = clf.predict(X_test)
                elif USE_TF and m == "Keras-MLP":
                    model = Sequential([Dense(128,activation="relu"),Dense(64,activation="relu"),Dense(1,activation="sigmoid")])
                    model.compile(optimizer="adam",loss="binary_crossentropy",metrics=["accuracy"])
                    model.fit(X_train,y_train,epochs=3 if not train_deep else 10,verbose=0)
                    pred = (model.predict(X_test).ravel()>0.5).astype(int)
                else:
                    continue

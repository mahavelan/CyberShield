...
if not uploaded_file:
    ...
    if dataset_type == "Labeled":
        ...
        if USE_TF:
            sup_choices += ["1D-CNN (train)", "Autoencoder-Finetune (not for classification)"]
        ...
        if USE_TF:
            unsup_choices += ["Autoencoder (train)"]
        ...
        if dataset_type == "Labeled":
            ...
            if "Random Forest" in chosen_sup:
                ...
                try:
                    ...
                    fi_df = pd.DataFrame(
                        {"feature": pipeline_info["feature_columns"], "importance": fi}
            ...
            if metrics_summary:
                metrics_df = pd.DataFrame.from_dict(
                    {m: {"Accuracy": v[0], "Precision": v[1], "Recall": v[2], "F1": v[3]} for m, v in metrics_summary.items()},
                    orient="index",
                )
            ...
            if "Consensus (hybrid)" in chosen_unsup:
                ...
                if not unsup_results:
                    ...
                    for name, dfm in unsup_results.items():
                        pcols = [c for c in dfm.columns if c.startswith("Pred_") or c == "Consensus_Pred"]
                        if pcols:
                            col = pcols[0]
                            cons_df[col] = dfm[col]
